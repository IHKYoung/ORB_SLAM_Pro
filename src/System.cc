...
namespace ORB_SLAM3
{
                                                                                         ...
                                                                                         mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
    {
        ...
        if (!node.empty() && node.isReal() && node.real() == 1.0)
        {
        ...
        else
        {
            ...
            // for Time Elapsed
            timeElapsed = clock() - startVocLoad;
            msElapsed = timeElapsed / (CLOCKS_PER_SEC / 1000);
            cout << "!! -- Load Vocabulary Time Elapsed " << msElapsed << " ms" << endl;
    ...
    clock_t startTrackStereo = clock();
                                     ...
                                     const vector<IMU::Point> &vImuMeas, string filename)
    {
        ...
        timeElapsed = clock() - startTrackStereo;
        msElapsed = timeElapsed / (CLOCKS_PER_SEC);
        // cout << "!! -- Resize Image Time Elapsed " << msElapsed << " μs" << endl;
        ...
        startTrackStereo = clock();
        Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed, imRightToFeed, timestamp, filename);
        timeElapsed = clock() - startTrackStereo;
        msElapsed = timeElapsed / (CLOCKS_PER_SEC / 1e6);
        // cout << "!! -- Calculate Tcw Time Elapsed " << msElapsed << " μs" << endl;
        ...
        timeElapsed = clock() - startTrackStereo;
        msElapsed = timeElapsed / (CLOCKS_PER_SEC / 1e6);
        // cout << "!! -- Tracking State Time Elapsed " << msElapsed << " μs" << endl;

        startTrackStereo = clock();
        mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
        timeElapsed = clock() - startTrackStereo;
        msElapsed = timeElapsed / (CLOCKS_PER_SEC / 1e6);
        // cout << "!! -- Tracked Map Points Time Elapsed " << msElapsed << " μs" << endl;

        startTrackStereo = clock();
        mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
        timeElapsed = clock() - startTrackStereo;
        msElapsed = timeElapsed / (CLOCKS_PER_SEC / 1e6);
        // cout << "!! -- Tracked Key Points Un Time Elapsed " << msElapsed << " μs" << endl;

        return Tcw;
    }

    Sophus::SE3f System::TrackStereoStatus(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, int &trackingState,
                                           const vector<IMU::Point> &vImuMeas, string filename)
    {
        if (mSensor != STEREO && mSensor != IMU_STEREO)
        {
    ...
    Sophus::SE3f System::TrackRGBDStatus(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, int &trackingState,
                                         const vector<IMU::Point> &vImuMeas, string filename)
    {
    ...
    void System::SaveTrajectoryMO(const string &filename)
    {
        cout << endl
             << "Saving camera trajectory to " << filename << " ..." << endl;
        if (mSensor == MONOCULAR)
        {
            cerr << "ERROR: SaveTrajectoryMO cannot be used for monocular." << endl;
            return;
        }
             ...
             lit != lend; lit++, lRit++, lT++, lbL++)
        {
            ...
            Eigen::Matrix3f R_old = Twc_old.rotationMatrix();
            Eigen::Vector3f t_old = Twc_old.translation();

            // Y 顺时针 90
            Eigen::Matrix3f rotationMatrixY;
            rotationMatrixY = Eigen::AngleAxisf(M_PI_2, Eigen::Vector3f::UnitY());
            // 新X 逆时针 90
            Eigen::Matrix3f rotationMatrixX;
            rotationMatrixX = Eigen::AngleAxisf(-1.0 * M_PI_2, Eigen::Vector3f::UnitX());

            Eigen::Matrix3f R_new = rotationMatrixX * rotationMatrixY * R_old;
            Eigen::Vector3f t_new = rotationMatrixX * rotationMatrixY * t_old;
    ...
    void System::SaveKeyFrameTrajectoryMO(const string &filename)
    {
        cout << endl
        ...
        for (size_t i = 0; i < vpKFs.size(); i++)
        {
            ...
            Sophus::SE3f Twc_old = pKF->GetPoseInverse();
            // 调整坐标系, 保持右手系, x-front, y-left, z-up
            Eigen::Matrix3f R_old = Twc_old.rotationMatrix();
            Eigen::Vector3f t_old = Twc_old.translation();
