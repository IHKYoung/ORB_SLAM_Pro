...
namespace ORB_SLAM3
{
        ...
        else
        {
            ...
            if (mState == LOST)
            {
                ...
                if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
                    if (!pCurrentMap->isImuInitialized())
                    {
                        Verbose::PrintMess("Track lost before IMU initialisation, resetting...",
    ...
    bool Tracking::TrackReferenceKeyFrame()
    {
        ...
        // 匹配数目小于5，认为跟踪失败（初始是15）
        if (nmatches < 5)
        {
            cout << "Track Reference KeyFrame Error: Less than 5 matches!!\n";
            return false;
        }
    ...
    bool Tracking::TrackLocalMap()
    {
        ...
        else
        {
            if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU)
            {
                Verbose::PrintMess("TLocalMapping: PoseOptimization ", Verbose::VERBOSITY_DEBUG);
                Optimizer::PoseOptimization(&mCurrentFrame);
            }
            else
            {
                ...
                {
                    Verbose::PrintMess("TLocalMapping: PoseInertialOptimizationLastFrame ", Verbose::VERBOSITY_DEBUG);
                    inliers = Optimizer::PoseInertialOptimizationLastFrame(
                        &mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
                }
                else
                {
                    Verbose::PrintMess("TLocalMapping: PoseInertialOptimizationLastKeyFrame ", Verbose::VERBOSITY_DEBUG);
            ...
            for (int i = 0; i < mCurrentFrame.N; i++)
            {
                ...
                if (pMP)
                {
                    if (!pMP->isBad())
                    {
                        ...
                        for (map<KeyFrame *, tuple<int, int>>::const_iterator it = observations.begin(), itend = observations.end(); it != itend;
                             it++)
